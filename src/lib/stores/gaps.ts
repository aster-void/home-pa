...
import type { DayBoundaries, Event, Gap } from '../services/gap-finder.js';
 ...
 */
function convertCalendarEventToGapEvent(calendarEvent: CalendarEvent, targetDate: Date): Event | null {
 ...
 */
export const gapStats = derived(
  gaps,
  (gapList) => {
    const totalGapTime = gapList.reduce((sum, gap) => sum + gap.duration, 0);
    const largestGap = gapList.reduce((max, gap) => 
      gap.duration > max.duration ? gap : max, 
