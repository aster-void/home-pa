...
import { expandRecurrences, type ExpandedOccurrence as IcalOccurrence } from '../services/calendar/index.js';
...
/** Error message */
export const calendarError = derived(
  calendarStore,
  ($state) => $state.error
);
...
export const calendarActions = {
    ...
    try {
      ...
      calendarStore.update(s => {
        ...
        const occurrences = s.currentWindow
          ? calendarActions.expandRecurringEvents(newEvents, s.currentWindow.start, s.currentWindow.end)
          : s.occurrences;
   ...
   */
  async updateEvent(id: string, updates: Partial<Omit<Event, 'id'>>): Promise<boolean> {
    try {
    ...
    for (const event of events) {
      ...
      if (event.recurrence.type === 'RRULE' && event.recurrence.rrule) {
        try {
          ...
          const richOccurrences: ExpandedOccurrence[] = icalOccurrences.map((occ: IcalOccurrence, index: number) => {
            const occStart = occ.startDate;
            const occEnd = occ.endDate || new Date(occStart.getTime() + durationMs);
          ...
          console.warn(`[calendarStore] Failed to expand recurring event ${event.id}:`, error);
        }
      }
    }
