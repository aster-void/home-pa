<script lang="ts">
  ...
  function getUpcomingEventsWithLocation() {
    ...
    const evs = Array.isArray($eventsForSelectedDate)
  ...
  function generateTransitSuggestion(event: Event) {
    const startDate =
      eventStartToDate(event.start, new Date($selectedDate)) || new Date();
...
<BaseCard title="Transit Suggestions" class="transit-card">
  ...
  {#if transitSuggestions.length === 0}
    <div class="no-transit" role="status" aria-live="polite">
      ...
      <p class="no-transit-hint">
        住所つきのイベントを追加すると移動案内が表示されます。
      </p>
    </div>
  {:else}
      ...
      {#each transitSuggestions as suggestion (suggestion.event.id || suggestion.event.start)}
        <article
          ...
          aria-labelledby={"title-" +
            (suggestion.event.id || suggestion.event.start)}
        >
          <header class="event-header">
            <div class="event-info">
              <h3
                id={"title-" + (suggestion.event.id || suggestion.event.start)}
                class="event-title"
              >
              ...
              <div class="event-details">
                <div class="event-topline">
                  <time
                    class="event-time"
                    datetime={new Date(suggestion.event.start).toISOString()}
                  >
                    {formatTime(new Date(suggestion.event.start).toISOString())}
                  </time>
                  <span class="dot-sep" aria-hidden="true">•</span>
                  <span class="event-location"
                    >📍 {suggestion.event.address}</span
                  >
                </div>
                <div class="event-sub">
                  <span class="muted"
                    >予定の種類: {suggestion.event.timeLabel || "一般"}</span
                  >
                </div>
              </div>
            </div>
            ...
            <div class="time-until" aria-hidden="false">
              ...
              <div
                class="time-value"
                aria-label={`Time until event: ${formatDuration(suggestion.timeUntilEvent)}`}
              >
              ...
              >
                <div class="option-header">
                  ...
                  >
                    ...
                    <span class="option-distance" aria-hidden="true">
                      — {option.distance}</span
                    >
                  </div>
                  <div
                    class="option-duration"
                    aria-label={`Duration ${formatDuration(option.duration)}`}
                  >
