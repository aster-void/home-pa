<script lang="ts">
  ...
  let { showLog = $bindable(false), externalGaps = $bindable(null) }: Props =
    $props();
  ...
  function eventToArcPathWithLane(
    startA: number,
    endA: number,
    laneIndex: number,
  ) {
  ...
  function normalizeEventsForDate(
    list: MyEvent[],
    day: Date,
  ): { startA: number; endA: number; lane: number; ref: MyEvent }[] {
    ...
    const truncated = list
      ...
      .map((ev) => {
        ...
        if (ev.timeLabel === "all-day") {
          ...
          return {
            ref: ev,
            startA: timeStringToAngle(startStr),
            endA: timeStringToAngle(endStr),
          };
        }
    ...
    const timedEvents = truncated.filter(
      (ev) => ev.ref.timeLabel !== "all-day",
    );
    const allDayEvents = truncated.filter(
      (ev) => ev.ref.timeLabel === "all-day",
    );
    ...
    const maxTimedLane =
      timedPlaced.length > 0 ? Math.max(...timedPlaced.map((p) => p.lane)) : -1;
  ...
  let renderedEvents = $derived(
    containerWidth > 0 && containerHeight > 0 && radius > 0
...
<div bind:this={containerElement} class="circular-timeline-container">
  ...
  >
    ...
    {#if containerWidth > 0 && containerHeight > 0 && radius > 0}
      ...
      {#each renderedEvents as ev (ev.ref.id)}
        ...
        {@const endAdj =
          ev.endA < ev.startA ? ev.endA + fullCircleRadians : ev.endA}
        {@const toX = (a: number) =>
          centerX + pathData.ringRadius * Math.cos(a - Math.PI / 2)}
        {@const toY = (a: number) =>
          centerY + pathData.ringRadius * Math.sin(a - Math.PI / 2)}
        ...
        {#if isAllDayInnermost}
          ...
          {@const laneStep = 0.07}
          {@const maxLaneCheck = Math.max(
            ...renderedEvents.map((e) => e.lane),
            0,
          )}
          {@const innermostEventRadiusCheck =
            radius * (0.85 - maxLaneCheck * laneStep)}
          {@const innerRadius =
            innermostEventRadiusCheck < defaultInnerRadius
              ? innermostEventRadiusCheck
              : defaultInnerRadius}
          {@const outerRadius = pathData.ringRadius}
          {@const innerStartX =
            centerX + innerRadius * Math.cos(ev.startA - Math.PI / 2)}
          {@const innerStartY =
            centerY + innerRadius * Math.sin(ev.startA - Math.PI / 2)}
          {@const innerEndX =
            centerX + innerRadius * Math.cos(endAdj - Math.PI / 2)}
          {@const innerEndY =
            centerY + innerRadius * Math.sin(endAdj - Math.PI / 2)}
    ...
    {#if containerWidth > 0 && containerHeight > 0 && radius > 0}
      ...
      {@const innerRadius =
        innermostEventRadius < defaultInnerRadius
          ? innermostEventRadius
          : defaultInnerRadius}
  ...
  {#if containerWidth > 0 && containerHeight > 0 && radius > 0}
    <div
      ...
      style={`left: ${centerX}px; top: ${centerY - 30}px;`}
    >
  ...
  {#if hoveredEvent}
    <div
      class="event-tooltip"
      style={`left: ${centerX + radius * 0.6}px; top: ${centerY - 20}px;`}
    >
      <div class="tooltip-content">
        <h4>{hoveredEvent.title}</h4>
        <p>
          {dateToHM(new Date(hoveredEvent.start))} - {dateToHM(
            new Date(hoveredEvent.end),
          )}
        </p>
  ...
  {#if hoveredGap}
    <div
      class="gap-tooltip"
      style={`left: ${centerX + radius * 1.1}px; top: ${centerY - 20}px;`}
    >
      <div class="tooltip-content">
        <h4>Free Time</h4>
