...
import type { ExpandedOccurrence } from "./calendar.svelte.ts";
...
import { eventFormState } from "./forms/eventForm.svelte.ts";
...
export const showEventForm = {
  ...
  set: (value: boolean) =>
    value ? uiState.openEventForm() : uiState.closeEventForm(),
  update: (fn: (value: boolean) => boolean) => {
    const newVal = fn(uiState.showEventForm);
    newVal ? uiState.openEventForm() : uiState.closeEventForm();
  },
};
...
export const showTimelinePopup = {
  ...
  set: (value: boolean) =>
    value ? uiState.openTimelinePopup() : uiState.closeTimelinePopup(),
  update: (fn: (value: boolean) => boolean) => {
    const newVal = fn(uiState.showTimelinePopup);
    newVal ? uiState.openTimelinePopup() : uiState.closeTimelinePopup();
  },
};
