...
describe("RecurrenceManager", () => {
  ...
  describe("Basic Event Creation", () => {
    ...
    it("should create a weekly recurring event", async () => {
      ...
      occurrences.forEach((occ) => {
        const dt = DateTime.fromJSDate(occ.startUtc, {
  ...
  describe("Weekly Bitmask Optimization", () => {
    ...
    it("should respect interval in weekly bitmask (every 2 weeks)", async () => {
      ...
      for (let i = 1; i < occurrences.length; i++) {
        const diff =
          occurrences[i].startUtc.getTime() -
          occurrences[i - 1].startUtc.getTime();
    ...
    it("should exclude occurrences matching EXDATE", async () => {
      const event = await manager.createEvent({
        ...
        exdateUtc: [
          ...
          new Date(
            DateTime.fromISO("2025-10-07T09:00:00", {
              zone: "America/New_York",
            })
              .toUTC()
              .toISO()!,
  ...
  describe("Overrides", () => {
    it("should cancel a single occurrence", async () => {
      ...
      const oct8 = occurrences.find(
        (occ) => occ.originalLocalISO === "2025-10-08T09:00:00",
      );
    ...
    it("should move occurrence inside window", async () => {
      ...
      const newStartUtc = DateTime.fromISO("2025-10-07T14:00:00", {
        zone: "America/New_York",
      }).toJSDate();
    ...
    it("should move occurrence outside window (remove from results)", async () => {
      ...
      const newStartUtc = DateTime.fromISO("2025-11-07T09:00:00", {
        zone: "America/New_York",
      }).toJSDate();
    ...
    it("should change duration with override", async () => {
      ...
      const newStartUtc = DateTime.fromISO("2025-10-07T09:00:00", {
        zone: "America/New_York",
      }).toJSDate();
      ...
      const duration =
        modified!.endUtc.getTime() - modified!.startUtc.getTime();
  ...
  describe("Timezone Handling", () => {
    ...
    it("should handle multiple events in different timezones", async () => {
      ...
      const nyOcc = occurrences.find((o) =>
        o.originalLocalISO.includes("10-15"),
      );
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle events with no duration", async () => {
      ...
      expect(occurrences).toHaveLength(1);
      expect(occurrences[0].startUtc.getTime()).toBe(
        occurrences[0].endUtc.getTime(),
      );
    });

    it("should deduplicate RDATE and generated occurrences", async () => {
      const startUtc = DateTime.fromISO("2025-10-06T09:00:00", {
        zone: "America/New_York",
      }).toJSDate();
