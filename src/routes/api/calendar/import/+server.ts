...
import { parseICS, parsedEventToDbCreate } from '$lib/services/calendar/index.js';
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    if (contentType.includes('text/calendar')) {
      ...
      throw error(400, 'Invalid content type. Expected text/calendar or application/json');
    }
    ...
    try {
      ...
      throw error(400, `Failed to parse ICS: ${parseError instanceof Error ? parseError.message : 'Unknown error'}`);
    }
    ...
    for (const parsed of parsedEvents) {
      try {
        ...
        const errorMsg = eventError instanceof Error 
          ? eventError.message 
          : 'Unknown error';
